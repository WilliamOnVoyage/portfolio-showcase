...
7. **Default Diagram Standard**: Always use the ByteByteGo System Architecture Infographic style (`ArchitectureDiagram.tsx`) for system architecture & design diagrams in blog posts (tagged via ```architecture, ```diagram, ```flow, or ```bytebytego).
